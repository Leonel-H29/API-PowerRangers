...
class CrudCapitulos():
    ...
    def exist_capitulo(cap,temp):
        cursor = conn.cursor()
        subquery= "SELECT id_temporada FROM temporadas WHERE numero_temporada={0}".format(temp) 
        query = "SELECT * FROM capitulos WHERE numero_cap={0} AND id_temporada=({1})".format(cap,subquery)
        try:
    ...
    def get_capitulos() : 
        ...
        for i in range(1, sheet.nrows):
            ...
            subquery = "SELECT id_temporada FROM temporadas WHERE numero_temporada={0}".format(col5)
            if exist==True:
                set = "numero_cap={0},nombre='{1}',descripcion='{2}',updated='{3}',id_temporada=({4}) ".format(col1,col2,col3,fyh,subquery)
                put_capitulos(col1, subquery ,set)
            else:    
                values = "({0},'{1}','{2}','{3}','{4}',({5})),".format(col1,col2,col3,fyh,fyh,subquery)
    ...
    def put_capitulos(n, data):
        cursor = conn.cursor()
        #print(data)
        query = "UPDATE capitulos SET {0} WHERE numero_cap={1} AND id_temporada=({2})".format(data,n,temp)  
        #print(query)
        try:
            cursor.execute(query)
