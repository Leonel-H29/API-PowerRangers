...
class CrudAparecen():
    ...
    def get_apariciones() : 
        ...
        for i in range(1, sheet.nrows):
            ...
            if exist==True:
                #pass
                set = "rol='{0}', descripcion='{1}', id_personaje={2}, id_temporada={3}".format(col3,col4, IDPer, IDTemp)
                put_aparecen(getIdAparicion(IDPer,IDTemp),set)
            else:   
                values = "('{0}','{1}',{2},{3}),".format(col3,col4, IDPer, IDTemp)
                #print(values)
    ...
    def put_apariciones(n, data):
        cursor = conn.cursor()
        #print(data)
        query = "UPDATE aparecen SET {0} WHERE id_aparicion={1};".format(data,id)  
        #print(query)
        try:
            #pass
            cursor.execute(query)
